...
CodecModule::CodecModule()
    : OperateInterface(),
      Task(),
      mDecID{AV_CODEC_ID_MJPEG},
      mEncID{AV_CODEC_ID_H264},
      mDec{},
      mEnc{},
      mDecCtx{},
      mEncCtx{},
...
CodecModule::CodecModule(const FrameInfo &frameCap,
...
void CodecModule::set(const FrameInfo &frameCap,
                      std::weak_ptr<FrameQueue> que) {
  mCap = frameCap;
...
int CodecModule::applyDecoder() {
  ...
  mDecCtx->width = mCap.width();
  mDecCtx->height = mCap.hight();
  mDecCtx->pix_fmt = AVPixelFormat(mCap.format());
...
int CodecModule::applyEncoder() {
  ...
  mEncCtx->width = mCap.width();
  mEncCtx->height = mCap.hight();
  mEncCtx->bit_rate = 0.96 * 1000000;
  mEncCtx->framerate = (AVRational){mCap.fps(), 1};
  mEncCtx->time_base = (AVRational){1, mCap.fps()};
...
int CodecModule::codecFrame(std::string &frame) {
  ...
  if (nullptr == mDecCtx or nullptr == mEncCtx) {
...
int CodecModule::decode() {
  if (nullptr == mDecCtx or nullptr == mDecPkt or nullptr == mDecFrm) {
  ...
  while (ret >= 0) {
    ...
    if (AVERROR(EAGAIN) == ret or AVERROR_EOF == ret) {
...
int CodecModule::swsCode() {
  ...
  mSwsCtx = sws_getContext(mDecFrm->width, mDecFrm->height,
                           AVPixelFormat(mDecFrm->format), mEncCtx->width,
                           mEncCtx->height, mEncCtx->pix_fmt, SWS_BILINEAR,
                           NULL, NULL, NULL);
